...
class __Emitter:
    ...
    def emit(self, sender, name, *args, **kwargs):
        ...
        """ Emit the signal @name from the @sender.

        The given @args and @kwargs are _appended_ to the
        receiver-specific arguments which have been specified via
        'connect'. If for example the callback `my_callback` was
        connected to the signal 'a-signal' of sender `an_object` like
        this:

        >>> connect(an_object, 'a-signal', my_callback, 'parrot', volts=500)

        And if the object `an_object` emitted its signal like this:

        >>> emit(self, 'a-signal', 'dead', volts=1000)

        Then the resulting arguments to my_callback would be an_object

           (the sender), 'dead', 'parrot', volts = 500.
    ...
    def list_signals(signals):
        """ Dump the given signals via 'print'. """
