...
# A 'DockBox' contains a list of 'Dock's  which are arranged horizontally.
# Each vbox may contain several DockBook's which themselves contain
# the actual 'Dockable's
...
class Dockbox(gtk.HBox):
    ...
    __gsignals__ = {
        'item-added' : (gobject.SIGNAL_RUN_FIRST , gobject.TYPE_NONE,
                        (gobject.TYPE_OBJECT,)),
        'item-removed' : (gobject.SIGNAL_RUN_FIRST , gobject.TYPE_NONE,
                          (gobject.TYPE_OBJECT,))
        }


    def __init__(self, cast):
        gtk.HBox.__init__(self, False) # False? How to avoid spacing?
        ...
        self.connect('item-added', self.item_added)
        self.connect('item-removed', self.item_removed)



    def add(self, item):
        self.add_book(self.cast(item))


    def item_added(self, sender, item):
        pass

    def item_removed(self, sender, item):
        pass
    ...
    def add_item(self, item, index=-1):
        assert( isinstance(item, Dockbook) )
        ##assert( item.parent == None )  # !!!! was: dockbook.dock
        ...
        if old_length == 0:
            ...
            if index == 0:
                paned.pack1(item, False, False)
                paned.pack2(old_item, False, False)
            else:
                paned.pack1(old_item, False, False)
                paned.pack2(item, False, False)
    ...
    def remove_item(self, item):
        ...
        if old_length == 1:
            ...
            separator = children[2]
            gtk.VBox.remove(self, separator)
            gtk.VBox.remove(self, item)
        else:
            parent = item.parent
            grandparent = parent.parent

            if index == 0:
                other_item = parent.get_child2()
            else:
                other_item = parent.get_child1()

            parent.remove( other_item )
            parent.remove( item )

            grandparent.remove( parent )

            if isinstance(grandparent, gtk.VPaned):
                grandparent.pack1(other_item, True, False)
            else:
                self.pack_start(other_item, True, True)                
